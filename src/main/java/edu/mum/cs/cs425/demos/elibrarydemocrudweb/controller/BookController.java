...
//    @Autowired
    ...
    public String searchBook(@PathVariable Integer bookId, Model model) {
       Book book= bookService.getBookById(bookId);
    ...
    @PostMapping("/elibrary/book/search")
    public String searchbook(@RequestParam("booktitle") String title, Model model){
        model.addAttribute("books", bookService.findBookByTitleContaining(title));
